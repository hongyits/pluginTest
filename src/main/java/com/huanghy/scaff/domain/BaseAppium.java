...
import com.huanghy.scaff.util.AndroidDriverWait;
...
@Listeners({com.huanghy.scaff.listener.AssertionListener.class})
public class BaseAppium {
    //调试设备名字
    public static String deviceName = "127.0.0.1:62001";
    //平台名称
    public static String platformName = "Android";
    //调试设备系统版本
    public static String platformVersion = "7.1.2";
    //app路径
    public static String appPath = System.getProperty("user.dir") + "/src/main/java/apps/shouhu2.2.3.apk";

    //包名
    public static String appPackage = "com.sina.weibo";
    ...
    //要启动的Activity
    public static String appActivity = appPackage + ".MainTabActivity";
...
//        driver.swipe(width / 2, height / percent, width / 2, height * (percent - 1) / percent, during);
        Duration duration = Duration.ofSeconds(during);
        int width = driver.manage().window().getSize().width;
        int height = driver.manage().window().getSize().height;
        TouchAction action2 = new TouchAction(driver).press(PointOption.point(width / 2, height / percent)).waitAction(WaitOptions.waitOptions(duration))
                .moveTo(PointOption.point(width / 2, height * (percent - 1) / percent)).release();
        action2.perform();
